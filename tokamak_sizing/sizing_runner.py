...
Run with:
    python3 -m tokamak_sizing.sizing_runner
"""
...
def print_result(result: SizingResult, inputs: SizingInputs, ref_name: str, r_ref: float):
    ...
    print(f"  Blanket     = {result.blanket_thickness_m:8.3f} m       (TBR={inputs.tbr_target}, local={tbr_local:.3f}, f_hole={inputs.f_hole:.0%}, {inputs.blanket_type})")
...
def run_st_sweep():
    ...
    for p_net in [50, 100, 150, 200, 250]:
        ...
        print(f"  {p_net:6.0f}  {result.r_major_m:6.2f}  {r_floor:7.2f}  {inputs.b_t:5.02f}  {result.delta_tf_m:8.3f}  {inb_req:7.02f}  {r_inboard:7.02f}  {result.binding_constraint}")
